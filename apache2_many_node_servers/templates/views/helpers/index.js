...
module.exports = function () {
		...
		function createTagList (tags) {
			...
			if (autolink) {
				return _.map(tags, function (tag) {
					return linkTemplate({
						url: ('/community-planner/blog/' + tag.key),
	...
	_helpers.isAdminEditorCSS = function (user, options) {
		...
		if (typeof (user) !== 'undefined' && user.isAdmin) {
			output = cssLinkTemplate({
				href: '/community-planner/keystone/styles/content/editor.min.css',
			});
		}
		return new hbs.SafeString(output);
	};
	...
	_helpers.isAdminEditorJS = function (user, options) {
		...
		if (typeof (user) !== 'undefined' && user.isAdmin) {
			output = scriptTemplate({
				src: '/community-planner/keystone/js/content/editor.js',
			});
		}
		return new hbs.SafeString(output);
	};
	...
	_helpers.postUrl = function (postSlug, options) {
		return ('/community-planner/blog/post/' + postSlug);
	};

	// might be a ghost helper
	// used for pagination urls on blog
	_helpers.pageUrl = function (pageNumber, options) {
		return '/community-planner/blog?page=' + pageNumber;
	};

	// create the category url for a blog-category page
	_helpers.categoryUrl = function (categorySlug, options) {
		return ('/community-planner/blog/' + categorySlug);
	};
